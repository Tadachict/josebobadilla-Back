...
router.get('/stats', authMiddleware, async (_req, res: Response): Promise<void> => {
  try {
    const [[p]]  = await pool.query('SELECT COUNT(*) as c FROM pages') as any
    const [[w]]  = await pool.query('SELECT COUNT(*) as c FROM works') as any
    const [[c]]  = await pool.query('SELECT COUNT(*) as c FROM comments') as any
    const [[g]]  = await pool.query('SELECT COUNT(*) as c FROM guestbook') as any
    const [[ic]] = await pool.query('SELECT COUNT(*) as c FROM iconografia') as any
    res.json({ pages: p.c, works: w.c, comments: c.c, guestbook: g.c, iconografia: ic.c })
...
router.put('/iconografia/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  const { title, image_url, caption, year, order_index } = req.body
  await pool.query('UPDATE iconografia SET title=?,image_url=?,caption=?,year=?,order_index=? WHERE id=?',
...
router.put('/curriculum/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  const { period, description, institution, category, order_index } = req.body
  await pool.query('UPDATE curriculum SET period=?,description=?,institution=?,category=?,order_index=? WHERE id=?',
...
router.put('/bibliografia/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  const { author, title, publisher, year, notes, order_index } = req.body
  await pool.query('UPDATE bibliografia SET author=?,title=?,publisher=?,year=?,notes=?,order_index=? WHERE id=?',
...
router.post('/comments', authMiddleware, async (req, res: Response): Promise<void> => {
  ...
  const [r] = await pool.query('INSERT INTO comments (author,email,body,source,is_published) VALUES (?,?,?,?,?)',
    [author, email||'', body, source||'', is_published?1:0]) as any
  res.status(201).json({ id: r.insertId })
})
router.put('/comments/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  const { author, email, body, source, is_published } = req.body
  await pool.query('UPDATE comments SET author=?,email=?,body=?,source=?,is_published=? WHERE id=?',
...
router.put('/guestbook/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  await pool.query('UPDATE guestbook SET is_approved=? WHERE id=?', [req.body.is_approved?1:0, req.params.id])
  res.json({ ok: true })
})
...
router.post('/nav', authMiddleware, async (req, res: Response): Promise<void> => {
  ...
  try {
    const [r] = await pool.query('INSERT INTO nav_items (label,slug,order_index,is_bold,is_active) VALUES (?,?,?,?,?)',
      [label, slug, order_index||0, is_bold?1:0, is_active?1:0]) as any
    res.status(201).json({ id: r.insertId })
  } catch (e: any) { res.status(400).json({ error: e.message }) }
})
router.put('/nav/:id', authMiddleware, async (req, res: Response): Promise<void> => {
  const { label, slug, order_index, is_bold, is_active } = req.body
  await pool.query('UPDATE nav_items SET label=?,slug=?,order_index=?,is_bold=?,is_active=? WHERE id=?',
